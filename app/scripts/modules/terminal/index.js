'use strict';
angular.module('sbAdminApp', ['terminalService', 'sentencesService'])
	.controller(
		'terminalCtrl',
		function ($scope, $rootScope, $stateParams, baseService, terminalService, programService, sentencesService) {
			$scope.displayed = [];
			$scope.sp = {};
			if ($stateParams.id) {
			...
			terminalService.getDomainSrc({
				domain: $stateParams.id
			}, function (result) {
				$scope.root_citys = result.root_citys;
				$scope.root_terminalReslotions = result.root_terminalReslotions;
			})
			$scope.callServer = function (tableState) {
				baseService.initTable($scope, tableState, baseService.api.terminal + 'getTerminalPageList');
			}
			$scope.$on('emitCity', function (e, cityData) {
				$scope.sp.provinceCode = cityData.province;
				$scope.sp.city_no = cityData.city;
				$scope.sp.districtCode = cityData.district;
			...
			$scope.changeEnabled = function (item, index) {
				baseService.confirm(item.enabled == 0 ? '启用设备' : '停用设备', '您确定' + (item.enabled == 0 ? '启用' : '停用') +
					'设备：' + item.name + '?',
					function (ngDialog) {
						terminalService.changeEnabled({
							...
							if (item.enabled == 1) {
								$scope.displayed[index].enabled = 0;
							} else {
								$scope.displayed[index].enabled = 1;
							}
							$scope.displayed = [];
